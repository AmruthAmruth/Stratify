...
const EmployeeAllocationCard: React.FC<Props> = ({ employee, onManageAllocation, onManageForecast, canManage }) => {
    ...
    return (
        <div className="bg-white rounded-lg border border-[#dfdcef] p-5 hover:shadow-md transition-all duration-200">
            {/* Employee Info */}
            <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                    <h4 className="font-semibold text-[#2f2f2f] text-lg">{employee.name}</h4>
                    <p className="text-sm text-[#5a5a5a] mt-0.5">{employee.position}</p>
                </div>
                {canManage && (
                    <div className="flex gap-2">
                        {onManageAllocation && (
                            <button
                                ...
                                className="text-[#009063] hover:text-[#007a52] text-sm font-medium px-3 py-1 rounded hover:bg-[#e6f7f0] transition-colors"
            ...
            {forecast && (
                <div className="mb-4 p-3 bg-[#f8f7fc] rounded-lg border border-[#e9e6f5]">
                    ...
                    {vsActual && (
                        <div className="space-y-2 mt-3">
                            ...
                            {leaveHours > 0 && (
                                <div className="flex justify-between items-center text-xs">
                                    <span className="text-[#5a5a5a]">Leave Impact:</span>
                            ...
                            {/* Utilization */}
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-[#5a5a5a]">Utilization:</span>
                                <span className="font-semibold text-[#2f2f2f]">{utilizationPercent.toFixed(0)}%</span>
                            </div>
                        </div>
                    )}
                    ...
                    {/* Forecast Period */}
                    {forecast.startDate && (
                        <div className="text-xs text-[#5a5a5a] mt-2 pt-2 border-t border-[#e9e6f5]">
            ...
            {allocation ? (
                <div className="space-y-4">
                    ...
                    <div>
                        <div className="flex justify-between items-center text-sm mb-2">
                            <span className="text-[#3b3b3b] font-medium">This Project</span>
                            <span className="font-bold text-[#2f2f2f] text-base">{allocationPercent}%</span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                            <div
                                className="bg-[#009063] h-2.5 rounded-full transition-all duration-300"
                    ...
                    <div>
                        <div className="flex justify-between items-center text-sm mb-2">
                            <span className="text-[#3b3b3b] font-medium">Total Capacity</span>
                    ...
                    {allocation.startDate && (
                        <div className="text-xs text-[#5a5a5a] pt-3 border-t border-[#e9e6f5]">
                ...
                <div className="text-center py-6 px-4 bg-[#fbfbfb] rounded-lg border border-dashed border-[#dfdcef]">
                    <svg className="w-10 h-10 mx-auto text-[#9b8dc9] mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p className="text-sm text-[#5a5a5a] font-medium">No allocation set</p>
                    <p className="text-xs text-[#5a5a5a] mt-1">Click "Manage" or "Forecast" to configure</p>
                </div>
            )}
        </div>
    );
