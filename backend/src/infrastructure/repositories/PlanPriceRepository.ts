...
async getPlan(plan: string): Promise<PlanPrice | null> {
    ...
    if (!doc) return null;
    return new PlanPrice(doc.plan,doc.description, doc.amount, doc.durationInMonths);
}


async setPlan(plan: string, description: string, amount: number, durationInMonths: number): Promise<void> {
 ...
 async listPlans(): Promise<PlanPrice[]> {
    ...
    return docs.map(doc => new PlanPrice(doc.plan,doc.description, doc.amount, doc.durationInMonths));
  }
  ...
  async updatePlan(plan: string, description: string, amount: number, durationInMonths: number): Promise<void> {
    await PlanPriceModel.updateOne(
      { plan },
      { description, amount, durationInMonths }
    );
